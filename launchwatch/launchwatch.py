...
class Holder:
    ...
    def is_sniper(self) -> bool:
        return self.first_seen > 0 and self.first_seen <= SNIPER_WINDOW_SECONDS
...
class LaunchMonitor:
    ...
    def on_trade(self, msg: dict):
        ...
        balance = float(msg.get("newTokenBalance") or 0)
        ...
        self.db.execute(
            ...
            (self.mint, datetime.now(timezone.utc).isoformat(), elapsed,
             ...
             int(wallet == self.creator), int(h.is_sniper)))
        # Commit per trade. The journal is the evidence — a hard kill must not
        # be able to lose the tail of it, and `report` reads its own
        # connection, so uncommitted rows would be invisible to it.
        self.db.commit()

    def _trim_flow(self, now: float):
    ...
    def top10_pct(self) -> float:
        ...
        held = sum(h.balance for h in sorted(active, key=lambda h: -h.balance)[:10])
        return held / total * 100

    def sniper_stats(self) -> tuple[int, float, float, float]:
        """(count, % of circulating held, % of total supply held, SOL extracted)"""
        snipers = [h for h in self.holders.values() if h.is_sniper]
        ...
        return len(snipers), held / total * 100, held / TOTAL_SUPPLY * 100, extracted

    def flow_ratio(self) -> tuple[float, float]:
        # Trim on read too — with no trades arriving, the window would
        # otherwise keep reporting stale pressure.
        self._trim_flow(time.time())
    ...
    def snapshot(self):
        active = self.active_holders()
        n_snipers, sniper_pct, sniper_supply_pct, extracted = self.sniper_stats()
        ...
        print(f"  sniper wallets   {n_snipers:>6}  holding {sniper_pct:.1f}%  "
              f"({sniper_supply_pct:.1f}% of supply)  "
    ...
    async def run(self):
        ...
        print("every trade is being journalled to launch.db\n")

        # Hold the reference — a bare create_task can be garbage collected
        # mid-flight, which silently kills the periodic snapshot.
...
def report(mint: str):
    ...
    try:
        # Reporting before the monitor has ever run against this DB_PATH is a
        # normal mistake (wrong directory, fresh clone). sqlite3.connect happily
        # creates an empty file, so check for the schema rather than letting the
        # first SELECT raise.
        if not db.execute("SELECT name FROM sqlite_master WHERE type='table'"
                          " AND name='events'").fetchone():
            print(f"no journal found at {DB_PATH} — run the monitor first")
            return
        ...
        print(f"  creator sold      {creator_sold or 0:.3f} SOL", end="")
        print("   <- publish this number" if not creator_sold
              else "   <- this will be found")

        print("\n  sniper cohort (first 30s):")
        for w, sol_in, sol_out, buys, sells in db.execute(
...
if __name__ == "__main__":
    if len(sys.argv) >= 3 and sys.argv[1] == "report":
        ...
        try:
            asyncio.run(LaunchMonitor(sys.argv[1], sys.argv[2]).run())
